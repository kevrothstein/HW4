...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/kevinHW4" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('tags', db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')), db.Column('search_term', db.String(32), db.ForeignKey('search_terms.term')))
...
user_collection = db.Table('user_collection', db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')), db.Column('collection_id', db.Integer, db.ForeignKey('personal_gif_collections.id')))
...
class User(UserMixin, db.Model):
    ...
    personal_gif_collections = db.relationship('PersonalGifCollection', backref='User')
...
class SearchTerm(db.Model):
    ...
    gifs = db.relationship('Gif', secondary=search_gifs, backref=db.backref('search_terms', lazy='dynamic'), lazy='dynamic')
...
def get_or_create_search_term(db_session, term, gif_list = []):
    ...
    if search_term:
        ...
        for result in search:
            gif = get_or_create_gif(db_session, title = result['title'], url = result['embed_url'])
...
def get_or_create_collection(db_session, name, current_user, gif_list):
    ...
    if collection:
        ...
        for gifies in gif_list:
            collection.gifs.append(gifies)
...
def create_collection():
    ...
    if request.method == "POST":
        ...
        get_or_create_collection(db.session, name=form.name.data, current_user=current_user, gif_list=gif_objects)
